...
Jedoch kann es trotzdem hilfreich sein die Kollisionen im Vorfeld zu Filtern. Dafür gibts es in der Klasse \keyword{CollisionGeometryOwner} die Mthode \ti{canCollideWith()}. Bevor eine Kollision ausgeführt wird, wird überprüft ob die beiden Geometrien auch wirklich miteinander kollidieren können. DIes ist auch wichtig für die \ti{movePosition()} Methode in der Klasse \keyword{Entity}. Ohne diese Methode würde das Entity mit jeder anderen CollisionBox kollidieren, mit ihr lässt sich dies auf die Objekte einschränken mit der das Entity auch wirklich zusammenstoßen soll.
