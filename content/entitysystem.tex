...
%\quickfigure{Lebenszyklus eines einzelnen Entities}{lifecycle_entity}{0.8} %TODO Image (onLayerAdd -> render | update -> repeat | remove)
...
\begin{itemize}
	\item \keyword{addFullCollisionBox():} Fügt ein Collision Rechteck hinzu mit gleichen Ausmaßen wie das Entity
	\item \keyword{addFullCollisionCircle():} Fügt ein Collision Kreis hinzu, welcher das Entity komplett ausfüllt
	\item \keyword{addFullCollisionTriangle(AlignCorner4 align):} Fügt ein Collision Dreieck hinzu, welches drei Ecken des Entities benutzt \ti{(Abhängig von dem übergebenen Alignment)}.
\end{itemize}
...
Man muss sich bei dieser Methode selbst darum kümmern dass die Bewegung unabhängig von der Updaterate ist, den auch mit nur halb so vielen Updates pro Sekunde sollte sich jedes Entity immer noch gleich schnell bewegen. Andernfalls wäre das Spiel nur auf einem Rechner mit gleichen oder ähnlichen UPS \ti{Updates per second} wie denen des Entwicklerrechners spielbar.
...
Das erste was wir uns klarmachen müssen ist, dass wir die Bewegungen bzw Beschleunigungen in X und Y Richtung unabhängig betrachten. Somit können wir das Problem auf eindimensionale herunterbrechen.
...
%\quickfigure{Spezielle ENtities}{special_entities}{0.8} %TODO Image (Class Diagramm)
